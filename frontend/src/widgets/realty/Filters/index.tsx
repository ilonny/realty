      ...
      <div>
        ...
        <FiltersContainer gap={5} paddingTop={0}>
          ...
          <Input
            ...
            value={districtsFilter.map((f) => f.name).join(", ")}
          />
          {!!(categoryId == 1) && (
            <Input
              readOnly
              ...
              value={seriesFilter.map((f) => f.name).join(", ")}
            />
          )}
          {!!(categoryId == 3) && (
            <Input
              ...
              value={apartmentComplexFilter.map((f) => f.name).join(", ")}
            />
          )}
          <Input
            readOnly
        ...
        <FiltersContainer gap={5} paddingTop={0}>
          ...
          <RangeWrapper>
            <p>
              Ценовой диапазон: <span>от $0 до $ 500,000</span>
            </p>
            <RangeSlider
            ...
            >
              ...
              <RangeSliderTrack bg="red.100">
                <RangeSliderFilledTrack bg={Colors.MAIN_RED} />
              </RangeSliderTrack>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Input
              ...
              onChange={(e) => setDistrictsSearchString(e.target.value)}
            />
            <br />
            <br />
                ...
                return (
                  <React.Fragment key={parent.id}>
                    <ModalItemWrapperBig>
                      <Checkbox
                        onChange={() => {
                          setDistrictsFilter((chosenAparts) => {
                            return chosenAparts.filter(
                              (ap) => ap.parent_id != parent.id
                            );
                          });
                          if (chosenDistrictParent.includes(parent.id)) {
                            setChosenDistrictParent((d) =>
                              d.filter((old) => old !== parent.id)
                            );
                          } else {
                            setChosenDistrictParent((d) => d.concat(parent.id));
                            setDistrictsFilter((chosenAparts) => {
                              const devAparts = districts.filter(
                                (a) => a.parent_id == parent.id
                              );
                              return chosenAparts.concat(devAparts);
                            });
                          }
                        }}
                        size={"lg"}
                        isChecked={chosenDistrictParent.includes(parent.id)}
                      >
                        {parent.name}
                      </Checkbox>
                    </ModalItemWrapperBig>
                    {districts
                      ...
                      .filter((d) => {
                        ...
                        return d?.name
                          ?.toLowerCase()
                          .includes(districtsSearchString.toLowerCase());
                      })
                      .map((district) => {
                        return (
                            ...
                            <Checkbox
                              ...
                              onChange={() => {
                                if (
                                  ...
                                  setDistrictsFilter((f) =>
                                    f.filter(
                                      (oldFilter) =>
                                        oldFilter.id !== district.id
                                    )
                                  );
                                } else {
                                  setDistrictsFilter((f) => f.concat(district));
                                }
                              }}
                            >
                              {district.name}
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Input
              ...
              onChange={(e) => setApartmentSearchString(e.target.value)}
            />
            <br />
            <br />
            <ModalFiltersContainer>
                ...
                return (
                  <React.Fragment key={dev.id}>
                    <ModalItemWrapperBig>
                      <Checkbox
                        onChange={() => {
                          setApartmentComplexFilter((chosenAparts) => {
                            return chosenAparts.filter(
                              (ap) => ap.developer_id != dev.id
                            );
                          });
                          if (chosenDev.includes(dev.id)) {
                            setChosenDev((d) =>
                              d.filter((old) => old !== dev.id)
                            );
                          } else {
                            setChosenDev((d) => d.concat(dev.id));
                            setApartmentComplexFilter((chosenAparts) => {
                              const devAparts = apartmentComplex.filter(
                        ...
                        size={"lg"}
                        isChecked={chosenDev.includes(dev.id)}
                      >
                        {dev.name}
                      </Checkbox>
                    </ModalItemWrapperBig>
                    {apartmentComplex
                      .filter((a) => a.developer_id == dev.id)
                      .filter((a) => {
                        ...
                        return a?.name
                          ?.toLowerCase()
                          .includes(apartmentSearchString.toLowerCase());
                      })
                      .map((ap) => {
                        return (
                          <ModalItemWrapper key={ap.id}>
                            <Checkbox
                              isChecked={
                              ...
                              onChange={() => {
                                if (
                                  ...
                                  setApartmentComplexFilter((f) =>
                                    f.filter(
                                      (oldFilter) => oldFilter.id !== ap.id
                                    )
                                  );
                                } else {
                                  setApartmentComplexFilter((f) =>
              ...
              {series.map((district) => {
                return (
                  <ModalItemWrapper key={district.id}>
                    <Checkbox
                      isChecked={
                        !!seriesFilter?.find((d) => d.id === district.id)
                      }
                      onChange={() => {
                        if (!!seriesFilter?.find((d) => d.id === district.id)) {
                          setSeriesFilter((f) =>
                            f.filter(
                              (oldFilter) => oldFilter.id !== district.id
                            )
                          );
                        } else {
                          setSeriesFilter((f) => f.concat(district));
                        }
                      }}
                    >
                      {district.name}
              ...
              {rooms.map((district) => {
                return (
                  <ModalItemWrapper key={district.id}>
                    <Checkbox
                      isChecked={
                        !!roomsFilter?.find((d) => d.id === district.id)
                      }
                      onChange={() => {
                        if (!!roomsFilter?.find((d) => d.id === district.id)) {
                          setRoomsFilter((f) =>
                            f.filter(
                              (oldFilter) => oldFilter.id !== district.id
                            )
                          ...
                          setRoomsFilter((f) => f.concat(district));
                        }
                      }}
                    >
                      {district.name}
              ...
              {state.map((district) => {
                return (
                  <ModalItemWrapper key={district.id}>
                    <Checkbox
                      isChecked={
                        !!stateFilter?.find((d) => d.id === district.id)
                      }
                      onChange={() => {
                        if (!!stateFilter?.find((d) => d.id === district.id)) {
                          setStateFilter((f) =>
                            f.filter(
                              (oldFilter) => oldFilter.id !== district.id
                            )
                          ...
                          setStateFilter((f) => f.concat(district));
                        }
                      }}
                    >
                      {district.name}
...
const CategoryFilter = styled(Flex)`
  ...
  padding-top: 0px;
  border-bottom: ${(props) =>
    props.active ? `2px solid ${Colors.MAIN_RED}` : `2px solid transparent`};
  color: ${(props) =>
