...
router.get("/", async function (req, res) {
    const list = await developer.findAll({ raw: true });
    res.json(list);
    return;
});
...
router.get("/get-one", async function (req, res) {
    try {
        const { id } = req.query;
        const model = await developer.findOne({
            where: { id },
        });
        res.json(model);
    } catch (e) {
...
router.post("/create", async (req, res) => {
    try {
        ...
        const data = await developer.create(dataToSave);
        res.json({
            success: true,
...
router.post("/update", async (req, res) => {
    try {
        const { id } = req.body;
        const existedModel = await developer.findOne({
            where: { id },
        });
