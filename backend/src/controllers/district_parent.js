...
router.get("/", async function (req, res) {
    const list = await district_parent.findAll({ raw: true });
    res.json(list);
    return;
});
...
router.get("/get-one", async function (req, res) {
    try {
        const { id } = req.query;
        const model = await district_parent.findOne({
            where: { id },
        });
        res.json(model);
    } catch (e) {
...
router.post("/create", async (req, res) => {
    try {
        ...
        const data = await district_parent.create(dataToSave);
        res.json({
            success: true,
...
router.post("/update", async (req, res) => {
    try {
        const { id } = req.body;
        const existedModel = await district_parent.findOne({
            where: { id },
        });
...
router.post("/delete", async (req, res) => {
    try {
        const { id } = req.body;
        await district_parent.destroy({ where: { id } });
        res.json({
            success: true,
