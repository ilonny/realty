...
router.get("/", async function (req, res) {
    ...
    if (currentUser.role === "admin") {
        list = await owner.findAll({ raw: true });
    } else {
        list = await owner.findAll({
            raw: true,
...
router.get("/get-one", async function (req, res) {
    try {
        const { id } = req.query;
        const model = await owner.findOne({
            where: { id },
        });
        res.json(model);
    } catch (e) {
...
router.post("/create", async (req, res) => {
    try {
        ...
        const data = await owner.create(dataToSave);
        res.json({
            success: true,
...
router.post("/update", async (req, res) => {
    try {
        const { id } = req.body;
        const existedModel = await owner.findOne({
            where: { id },
        });
